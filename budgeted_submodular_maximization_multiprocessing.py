...
def worker(worker_id, submodularity_param):
    # make a copy of rouge folder for each worker
    path_to_rouge_of_worker = '/tmp/takahe/rouge2.0-distribution_' + str(worker_id) + '/'
...
summary_size_range = [350] if dataset_id == 'ami' else [450]
    ...
    for system_name in system_name_list:
        ...
        for MSC_param_id in range(len(MSC_system_params_dict[system_name])):
            ...
            for submodularity_param_id, submodularity_param in enumerate(submodularity_params):
                pool_results.append(
                    pool.apply_async(worker, args=(submodularity_param_id, submodularity_param))
                )
            ...
            for pool_result in pool_results:
                submodularity_param_id, overall_evaluation_score, scores = pool_result.get()
            ...
            with open(path_to_root + system_name + '_evaluation.csv', "a") as f:
                for submodularity_param_id, submodularity_param in enumerate(submodularity_params):
                    f.write(
                        str(corpus_id) + ',' +
                        str(MSC_param_id) + ',' +
                        str(submodularity_param_id) + ',' +
                        str(overall_evaluation_scores_of_submodularity_params[submodularity_param_id])
                    )

                    for key in ['Avg_F-Score', 'Avg_Precision', 'Avg_Recall']:
                        for summary_size in summary_size_range:
                            f.write(',')
                            f.write(str(scores_of_submodularity_params[submodularity_param_id][str(summary_size)][key]))
                    f.write('\n')
